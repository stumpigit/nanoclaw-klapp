...
async function runScript(script, args = {}) {
    ...
    return new Promise((resolve) => {
        ...
        proc.on('close', (code) => {
            clearTimeout(timer);
            if (stderr)
                logger.debug({ script, stderr: stderr.slice(0, 500) }, 'Klapp script stderr');
            try {
 ...
 */
export async function handleKlappIpc(data, sourceGroup, _isMain, dataDir) {
    const type = data.type;
    if (!type?.startsWith('klapp_'))
        return false;
